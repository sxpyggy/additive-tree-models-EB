...
train_ind<-1:3;valid_ind<-4;test_ind<-5
...
dtrain_poi<-
  xgb.DMatrix(data=train_mat, label = dat_train$ClaimNb,
                        base_margin=log(dat_train$Exposure))
dvalid_poi<-xgb.DMatrix(data=valid_mat, label = dat_valid$ClaimNb,
                        base_margin=log(dat_valid$Exposure))

dtrain_gam<-
  xgb.DMatrix(data=train_mat[dat_train$Y>0,], label = dat_train$Y[dat_train$Y>0], base_margin=log(dat_train$ClaimNb[dat_train$Y>0]/dat_train$Exposure[dat_train$Y>0]),
              weight = dat_train$ClaimNb[dat_train$Y>0])
dvalid_gam<-
  xgb.DMatrix(data=valid_mat[dat_valid$Y>0,], label = dat_valid$Y[dat_valid$Y>0], base_margin=log(dat_valid$ClaimNb[dat_valid$Y>0]/dat_valid$Exposure[dat_valid$Y>0]),
...
dat_learn$bst_lambda<-predict(bst_poisson,newdata=dlearn,type="response")
dat_learn$bst_tau<-predict(bst_gamma,newdata=dlearn,type="response")
dat_test$bst_lambda<-predict(bst_poisson,newdata=dtest,type="response")
dat_test$bst_tau<-predict(bst_gamma,newdata=dtest,type="response")
...
## tw_bst
```{r}
param_tw<-list(max_depth=6, eta =0.1, objective="reg:tweedie", tweedie_variance_power=p_glm)
vnames_bst
...
tw_bst <-
  xgb.train(param_tw, dtrain_tw, nrounds=500, verbose = 1, watchlist = watchlist_poi, early_stopping_rounds = 5)
...
(loss_test_tw_bst<-mean(dat_test$logL_tw_bst));loss_test_tw_glm
```
...
loss_test_tw_bst
loss_test_tw_glm
loss_test_sum<-round(data.frame(loss_test_EB,loss_test_EM,loss_test_poi_gam_bst,loss_test_poi_gam_glm,loss_test_poi_gam_null,loss_test_tw_bst,loss_test_tw_glm),4)
write.csv(loss_test_sum,paste("./plots/3-test-loss-",test_ind,".csv",sep=""))
```
...
importance_lambda<-data.frame(matrix(NA,ncol=length(var_name),
                              nrow=which.min(eb_model$valid_loss)))
names(importance_lambda)<-var_name
importance_tau<-data.frame(matrix(NA,ncol=length(var_name),
                              nrow=which.min(eb_model$valid_loss)))
names(importance_tau)<-var_name
for (k in 1:which.min(eb_model$valid_loss)){
  boost_lambda<-eb_model$lambda_models[[k]]
  boost_tau<-eb_model$tau_models[[k]]
