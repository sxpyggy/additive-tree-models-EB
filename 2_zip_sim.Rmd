...
hist(dat$y,freq = F, main="",xlab="number of claims", ylab="frequency")
box()
...
round(cbind(loss_true,loss_null,loss_null2,loss_glm_lambda,loss_glm_pi,loss_glm_both),4)
```

# boosting
```{r}
valid_rows<-which(dat_learn$ind==4)
pi0<-rep(null_mle$pi_hat, nrow(dat_learn))
lambda0<-rep(null_mle$lambda_hat, nrow(dat_learn))
M0<-200
n_tree_lambda<-1
maxdepth_lambda<-3
eta_lambda<-0.1
...
# bst_pi, bst_pi_lambda
structure<-"pi"
bst_pi<-EB_zip(dat_learn, valid_rows, lambda0, pi0, M0,
...
structure<-"lambda"
bst_pi_lambda<-EB_zip(dat_learn, valid_rows, bst_pi$lambda_hat, bst_pi$pi_hat, M0,
                          n_tree_lambda, maxdepth_lambda, eta_lambda,
                          n_tree_pi, maxdepth_pi, eta_pi,
                          structure, trace, patience)
matplot(cbind(bst_pi_lambda$train_loss,bst_pi_lambda$valid_loss),lty=c(1,1),col=c("red","blue"),type="l")

# bst_lambda, bst_lambda_pi
structure<-"lambda"
bst_lambda<-EB_zip(dat_learn, valid_rows, lambda0, pi0, M0,
                          n_tree_lambda, maxdepth_lambda, eta_lambda,
                          n_tree_pi, maxdepth_pi, eta_pi,
                          structure, trace, patience)
matplot(cbind(bst_lambda$train_loss,bst_lambda$valid_loss),lty=c(1,1),col=c("red","blue"),type="l")
structure<-"pi"
bst_lambda_pi<-EB_zip(dat_learn, valid_rows, bst_lambda$lambda_hat, bst_lambda$pi_hat, M0,
                          n_tree_lambda, maxdepth_lambda, eta_lambda,
                          n_tree_pi, maxdepth_pi, eta_pi,
                          structure, trace, patience)
...
valid_loss<-data.frame(round(cbind(min(bst_pi$valid_loss),
min(bst_pi_lambda$valid_loss),
min(bst_lambda$valid_loss),
min(bst_lambda_pi$valid_loss),
min(bst_both$valid_loss)),4))
names(valid_loss)<-c("pi","pi-lambda","lambda","lambda-pi","both")
if (draw_figure==T) write.csv(valid_loss,"./plots/2-zip-bst-loss.csv")
```

## bst_both, bst_lambda
```{r}
# bst_both
dat_test$lambda_bst<-rep(null_mle$lambda_hat, nrow(dat_test))
dat_test$pi_bst<-rep(null_mle$pi_hat, nrow(dat_test))
m_iter<-which.min(bst_both$valid_loss)
for (m in 1:m_iter){
...
dat_test$lambda_bst<-rep(null_mle$lambda_hat, nrow(dat_test))
dat_test$pi_bst<-rep(null_mle$pi_hat, nrow(dat_test))
m_iter<-which.min(bst_lambda$valid_loss)
for (m in 1:m_iter){
  dtest_lambda<-xgb.DMatrix(data=as.matrix(dat_test[,c("x1","x2","x3","x4","x5")]), base_margin=log(dat_test$lambda_bst))
...
# bst_poi
```{r}
param<-list(max_depth=3, eta =0.1, objective="count:poisson")
...
loss_mat<-data.frame(
  ...
  negL=c(loss_null,loss_glm_lambda,loss_glm_pi,loss_glm_both,loss_bst_lambda,loss_bst_both,NA,loss_true),error_pif=NA,error_eta=NA,loss0=NA)
...
loss_mat
loss_mat<-loss_mat[c(5,6,3,2,4,1,7,8),]
