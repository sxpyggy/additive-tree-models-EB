...
EM_zip <- function(dat_learn, M0, structure){
  ...
  if (structure=="both"){
    ...
    glm_ber<-glm(ZZ ~ x1 + x2 + x3 + x4 + x5, family = quasibinomial(link="logit"), data = dat_learn_aug, weights=w)
  }
  if (structure=="lambda"){
    glm_poi<-glm(y ~ x1 + x2 + x3 + x4 + x5, family = poisson(link="log"), data = dat_learn_aug[dat_learn_aug$ZZ==0,], weights=w)
    glm_ber<-glm(ZZ ~ 1, family = quasibinomial(link="logit"), weights=w, data = dat_learn_aug)
  }
  if (structure=="pi"){
