...
EM_zip <- function(dat_learn, M0, structure){
  # structure = "both", "lambda", "pi", "null"
  # dat_learn are learning data
  dat_learn_aug<-dat_aug(dat_learn)
  ...
  if (structure=="both"){
    glm_poi<-glm(y ~ x1 + x2 + x3 + x4 + x5, family = poisson(link="log"), data = dat_learn_aug[dat_learn_aug$ZZ==0,], weights=w)
    glm_ber<-glm(ZZ ~ x1 + x2 + x3 + x4 + x5, family = binomial(link="logit"), data = dat_learn_aug, weights=w)
  }
  if (structure=="lambda"){
    glm_poi<-glm(y ~ x1 + x2 + x3 + x4 + x5, family = poisson(link="log"), data = dat_learn_aug[dat_learn_aug$ZZ==0,], weights=w)
    glm_ber<-glm(ZZ ~ 1, family = binomial(link="logit"), weights=w, data = dat_learn_aug)
  }
  if (structure=="pi"){
    glm_poi<-glm(y ~ 1, family = poisson(link="log"), weights=w, data = dat_learn_aug[dat_learn_aug$ZZ==0,])
    glm_ber<-glm(ZZ ~ x1 + x2 + x3 + x4 + x5, family = binomial(link="logit"), data = dat_learn_aug, weights=w)
  }
  if (structure=="null"){
    glm_poi<-glm(y ~ 1, family = poisson(link="log"), weights=w, data = dat_learn_aug[dat_learn_aug$ZZ==0,])
...
EB_zip <- function(dat_learn, valid_rows, lambda0, pi0, M0,
  ...
  for (m in 1:M0) {
    ...
    if (structure == "lambda"| structure == "both") {
      dtrain<-xgb.DMatrix(data=as.matrix(dat_train_aug[,c("x1","x2","x3","x4","x5")]), label = dat_train_aug$y, weight=dat_train_aug$lambda_w, base_margin= dat_train_aug$lambda_margin)
      dvalid<-xgb.DMatrix(data=as.matrix(dat_valid[,c("x1","x2","x3","x4","x5")]), base_margin= dat_valid$lambda_margin)
      dlearn<-xgb.DMatrix(data=as.matrix(dat_learn0[,c("x1","x2","x3","x4","x5")]), base_margin= dat_learn0$lambda_margin)
    }
    if (structure == "pi"){
    ...
    if (structure == "pi"| structure == "both") {
      dtrain<-xgb.DMatrix(data=as.matrix(dat_train_aug[,c("x1","x2","x3","x4","x5")]), label = dat_train_aug$ZZ, weight=dat_train_aug$w, base_margin= dat_train_aug$pi_margin)
      dvalid<-xgb.DMatrix(data=as.matrix(dat_valid[,c("x1","x2","x3","x4","x5")]), base_margin=dat_valid$pi_margin)
      dlearn<-xgb.DMatrix(data=as.matrix(dat_learn0[,c("x1","x2","x3","x4","x5")]), base_margin= dat_learn0$pi_margin)
    }
    if (structure == "lambda"){
